...
temp_file = "temp_perfect_graph.html"
...
<script type='text/javascript'>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // 關鍵修正 1: 首次繪製後，禁用階層佈局，鎖定節點
    network.on('afterDrawing', function () {
      network.setOptions({
        layout: { hierarchical: false },
        physics: { enabled: false } 
      });
      network.off('afterDrawing'); // 僅執行一次
    });

    const pathToggle = document.getElementById('pathToggle');
    pathToggle.addEventListener('change', function() {
      const showFullPath = this.checked;
      const currentPositions = network.getPositions(); // 獲取使用者拖曳後的最新位置
      let updatedNodes = [];
      ...
      nodes.forEach(node => {
        ...
        // 關鍵修正 2: 更新時，強制使用當前座標，並設定 fixed:true
        updatedNodes.push({
          id: node.id,
          label: newLabel,
          x: position.x,
...
final_file = "perfect_interactive_graph.html"
with open(final_file, 'w', encoding='utf-8') as f:
    f.write(html_content)

os.remove(temp_file)

print(f"Successfully generated perfect interactive graph at: {os.path.join(os.getcwd(), final_file)}")