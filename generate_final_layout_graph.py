...
temp_file = "temp_final_layout_graph.html"
...
<script type='text/javascript'>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    pathToggle.addEventListener('change', function() {
      ...
      const currentPositions = network.getPositions(); // 獲取所有節點當前的位置
      let updatedNodes = [];
      ...
      nodes.forEach(node => {
        ...
        // 在更新中明確指定節點的 x 和 y 座標，並設定 fixed 為 true
        updatedNodes.push({
          id: node.id,
          label: newLabel,
          x: position.x,
          y: position.y,
          fixed: true // 確保節點在更新後保持固定
        });
      });
...
final_file = "final_layout_graph.html"
with open(final_file, 'w', encoding='utf-8') as f:
    f.write(html_content)

os.remove(temp_file)

print(f"Successfully generated final layout graph at: {os.path.join(os.getcwd(), final_file)}")
